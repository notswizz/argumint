...
export default function ArenaPage() {
  ...
  const handleTabChange = (tab) => {
    ...
    router.push(`/arena?tab=${tab}`, undefined, { shallow: true });
  };
  ...
  const createTake = async (e) => {
    ...
    try {
      ...
      setNewStatement('');
      setSuccess('Take created successfully!');
      setActiveSubTab('train'); // Switch to train sub-tab after creation
      mutate();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const sendInterview = async (e) => {
    ...
    try {
      const res = await fetch('/api/arena/interview', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ takeId: selected._id, userMessage: userMessage }),
      });
  ...
  const finalizeProfile = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text();
        throw new Error(errorText || 'Failed to train AI');
      }
      
      setSuccess('AI trained successfully!');
      mutate();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const gradeDebate = async () => {
    ...
    try {
      ...
      setSuccess('Debate graded successfully!');
      mutate();
    } catch (e) {
      setError(e.message);
    } finally {
      ...
      <div className="px-4 sm:px-6 pb-20 sm:pb-6">
        ...
        <div className="max-w-6xl mx-auto mb-16">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Create & Train Your AIs</h2>
            <p className="text-slate-600">Start with a statement and train an AI to understand your position</p>
          </div>
                    ...
                    {!showChat && (
                      <div className="space-y-4">
                        <div className="text-center">
                          <h4 className="font-medium text-slate-700 mb-2">Chat with AI</h4>
                          <p className="text-sm text-slate-600">Answer questions or add details to train your AI</p>
                        </div>
        ...
        <div className="max-w-6xl mx-auto mb-16">
          <div className="flex items-center">
            ...
            <div className="px-6 text-slate-400 text-sm font-medium">Ready to Debate?</div>
            <div className="flex-1 border-t border-slate-200"></div>
          </div>
        </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-6 shadow-sm space-y-4">
                  ...
                  <div className="rounded-xl border border-slate-200 p-4 bg-slate-50">
                    ...
                    {!showChat && (
                      <div className="space-y-4">
                        <div className="text-center">
                          <h4 className="font-medium text-slate-700 mb-2">Send Your Argument</h4>
                          <p className="text-sm text-slate-600">Type your message and see how the AI responds</p>
                        </div>
                        ...
                        <form onSubmit={async (e) => {
                          ...
                          try {
                            ...
                            const data = await res.json();
                            setOpponentMessage('');
                            setShowChat(true); // Switch back to chat view after sending
