...
export async function triggerBotReplies({ roomId, triadId, promptId, excludeUserId = null }) {
  try {
    ...
    for (const a of assignments) {
      ...
      let text = await generateBotReply({ personaKey: a.personaKey, promptText: prompt.text, transcript });
      // Human-like fallback if OpenAI not available or returned nothing
      if (!text) {
        // Pull a recent message to riff on
        const target = reversed.filter((m) => String(m.senderId) !== String(bot._id)).slice(-1)[0] || reversed[reversed.length - 1];
      ...
      // Add bot to room participants for avatar/name display (but NOT to triad participants)
      await ChatRoom.updateOne({ _id: roomId }, { $addToSet: { participants: bot._id } });

      // Persist message with derived analytics
      ...
      // Try to broadcast over Socket.IO if server is initialized
      try {
