...
export async function triggerBotReplies({ roomId, triadId, promptId, excludeUserId = null }) {
  try {
    ...
    const recent = await Message.find({ triadId }).sort({ createdAt: -1 }).limit(20).lean();
    ...
    for (const a of assignments) {
      ...
      if (lastBot && Date.now() - new Date(lastBot.createdAt).getTime() < 45 * 1000) continue;
      ...
      const text = await generateBotReply({ personaKey: a.personaKey, promptText: prompt.text, transcript });
      if (!text) continue;
