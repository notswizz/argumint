...
export const PERSONAS = {
  witty: {
    ...
    system: [
      ...
      'Always contribute a brief, helpful response. Do not output [SKIP].',
    ].join('\n'),
  },
  professor: {
    ...
    system: [
      ...
      'Read the prompt and chat transcript to tailor your response. Always contribute a brief, helpful response. Do not output [SKIP].',
    ].join('\n'),
  },
  trash: {
    ...
    system: [
      ...
      'Read the prompt and chat transcript and add spicy but constructive takes. Always contribute something constructive. Do not output [SKIP].',
    ].join('\n'),
  },
};

const client = process.env.OPENAI_API_KEY ? new OpenAI({ apiKey: process.env.OPENAI_API_KEY }) : null;
const BOT_DEBUG = /^(1|true|yes)$/i.test(process.env.BOT_DEBUG || '');
function dlog(...args) {
  if (BOT_DEBUG) {
    try { console.log('[BOT]', ...args); } catch {}
  }
}
...
async function generateBotReply({ personaKey, promptText, transcript }) {
  ...
  try {
    ...
    dlog('generateBotReply OK', { personaKey, len: text.length });
    if (!text || /\[\s*skip\s*\]/i.test(text)) return null;
    return text;
  } catch (e) {
    dlog('generateBotReply ERR', e?.message || e);
    return null;
  }
}
...
export async function generateBotStanceShort(personaKey, promptText) {
  ...
  try {
    ...
    const text = (res.choices?.[0]?.message?.content || '').trim();
    dlog('generateBotStanceShort OK', { personaKey, len: text.length });
    return text || null;
  } catch (e) {
    dlog('generateBotStanceShort ERR', e?.message || e);
    return null;
  }
}
...
export async function triggerBotReplies({ roomId, triadId, promptId, excludeUserId = null }) {
  try {
    await connectToDatabase();
    dlog('triggerBotReplies start', { roomId, triadId, promptId, excludeUserId });
    // Ensure triad is active
    const triad = await Triad.findById(triadId).lean();
    if (!triad || triad.status !== 'active') {
      dlog('triad not active or missing');
      return;
    }

    const prompt = await Prompt.findById(promptId).lean();
    if (!prompt) { dlog('prompt not found'); return; }
    ...
    const botParticipants = (triad.participants || [])
      ...
      .filter(Boolean);
    if (!botParticipants.length) { dlog('no bot participants in triad'); return; }

    // Load recent messages and map user ids to usernames
    const recent = await Message.find({ triadId }).sort({ createdAt: 1 }).limit(120).lean();
    ...
    dlog('context loaded', { messages: reversed.length, users: users.length });
    ...
    for (const b of botParticipants) {
      ...
      if (lastBotRecent && Date.now() - new Date(lastBotRecent.createdAt).getTime() < 1000) { dlog('skip recent bot post'); continue; }
      ...
      if (!shouldReplyDecision(b.personaKey, lines)) { dlog('decision skip'); continue; }
      ...
      if (lastBotNow2 && Date.now() - new Date(lastBotNow2.createdAt).getTime() < 400) { dlog('skip back-to-back guard'); continue; }
      const recentNow = await Message.find({ triadId }).sort({ createdAt: 1 }).limit(120).lean();
      ...
      if (!text) { dlog('no text after fallbacks'); continue; }
      ...
      if (lastBotCheck && isTooSimilar(text, lastBotCheck.content)) { dlog('skip similar to last bot'); continue; }
      const lastHuman = reversedNow.slice(-1)[0];
      if (lastHuman && isTooSimilar(text, lastHuman.content)) { dlog('skip similar to last human'); continue; }
      ...
      dlog('posted bot message', { len: text.length, sentiment, wordCount, roomId });
      try {
