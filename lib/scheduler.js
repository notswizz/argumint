...
export async function evaluateExpiredTriads() {
  ...
  for (const t of needsGrading) {
      ...
      try {
        ...
        if (result && Array.isArray(result.per_user) && result.per_user.length) {
          userScores = result.per_user
            ...
            .map((r) => {
              const d = Math.max(0, Math.min(100, Math.round(r.defense || 0)));
              const e = Math.max(0, Math.min(100, Math.round(r.evidence || 0)));
              const l = Math.max(0, Math.min(100, Math.round(r.logic || 0)));
              const rs = Math.max(0, Math.min(100, Math.round(r.responsiveness || 0)));
              const c = Math.max(0, Math.min(100, Math.round(r.clarity || 0)));
              const weighted = Math.max(0, Math.min(100, Math.round(0.35 * d + 0.20 * e + 0.20 * l + 0.15 * rs + 0.10 * c)));
              return ({
                userId: t.participants[r.userIndex],
                score: weighted,
                rubric: { defense: d, evidence: e, logic: l, responsiveness: rs, clarity: c },
    ...
    // Persist per-user and group feedback immediately
    // Also set score to the average of user scores for consistency across postings
    ...
    // Overall triad score and winner
    if (t.status === 'active') {
      let score = avgScore;
      if ((!Number.isFinite(score) || score <= 0) && client && transcript) {
        try {
          const promptDoc = await Prompt.findById(t.promptId).lean();
