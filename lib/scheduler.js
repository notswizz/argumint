  ...
  for (const p of duePrompts) {
    ...
    while (queue.length >= 3) {
      ...
      try {
        ...
        // Kick bots to produce an additional contextual reply after intros
...
export async function ensureFiveActivePrompts() {
  ...
  // Make staggering more granular to avoid simultaneous expiries
  const STAGGER_MS = Number(process.env.PROMPTS_STAGGER_MS || 60 * 1000); // default 1 minute
  const JITTER_MS = Number(process.env.PROMPTS_STAGGER_JITTER_MS || 20 * 1000); // default up to 20s random jitter

  // Compute the next deadline base: later of now+10m or latest existing + stagger
  const latest = active.length ? new Date(active[active.length - 1].scheduledFor).getTime() : 0;
  let nextDeadlineMs = Math.max(Date.now() + TEN_MIN, latest ? latest + STAGGER_MS : 0);
  ...
  while (active.length < 5 && attempts < 3) {
    ...
    for (let i = 0; i < toMake; i++) {
      ...
      docs.push({
        ...
        category: g.category,
        active: true,
        scheduledFor: new Date(nextDeadlineMs + Math.floor(Math.random() * JITTER_MS)),
      });
      nextDeadlineMs += STAGGER_MS;
    }
...
export async function evaluateExpiredTriads() {
  ...
  for (const t of needsGrading) {
    ...
    if (t.status === 'active') {
      ...
      await Triad.updateOne({ _id: t._id }, { score });
    }
  }
