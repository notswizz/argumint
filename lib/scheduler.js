...
export async function scheduleDuePrompts() {
  ...
  for (const p of duePrompts) {
    ...
    while (queue.length >= 3) {
      ...
      await Triad.create({ promptId: prompt._id, roomId: room._id, participants: participantsObj, startedAt: new Date(), status: 'active' });
    }
    if (queue.length === 2) {
...
export async function ensureFiveActivePrompts() {
  ...
  // Compute the next deadline base: later of now+10m or latest existing + 5m
  const latest = active.length ? new Date(active[active.length - 1].scheduledFor).getTime() : 0;
  let nextDeadlineMs = Math.max(Date.now() + TEN_MIN, latest ? latest + FIVE_MIN : 0);
  ...
  while (active.length < 5 && attempts < 3) {
    ...
    for (let i = 0; i < toMake; i++) {
      ...
      docs.push({
        ...
        category: g.category,
        active: true,
        scheduledFor: new Date(nextDeadlineMs),
      });
      nextDeadlineMs += FIVE_MIN;
    }
    if (docs.length) await Prompt.insertMany(docs);

    // Requery to confirm count and update latest schedule
    active = await Prompt.find({ active: true, scheduledFor: { $gt: new Date() } })
      .sort({ scheduledFor: 1 })
      .lean();
    attempts += 1;
  }
